-- Migration 042: Add optional gender and age_group filters to al_race_correlation_stats.
--
-- Must DROP and recreate because PostgreSQL does not allow adding parameters
-- to an existing function via CREATE OR REPLACE.

DROP FUNCTION IF EXISTS public.al_race_correlation_stats(UUID);

CREATE FUNCTION public.al_race_correlation_stats(
  p_race_id   UUID,
...
base AS (
  ...
  WHERE rr.race_id = p_race_id
    ...
    AND (p_gender    IS NULL OR rr.gender    = p_gender)
    AND (p_age_group IS NULL OR rr.age_group = p_age_group)
),
stats AS (
  SELECT
    COUNT(*)                                                                                 AS n_fin,
...
GRANT EXECUTE ON FUNCTION public.al_race_correlation_stats(UUID, TEXT, TEXT) TO authenticated;
