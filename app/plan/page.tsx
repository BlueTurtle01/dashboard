    ...
    return (
      <main className="min-h-screen bg-zinc-50 px-4 py-8 text-zinc-900">
        <div className="mx-auto max-w-2xl rounded-2xl border border-red-200 bg-red-50 p-6 shadow-sm">
          <p className="text-red-700">{error || "No plan found. Contact support if you think this is an error."}</p>
        </div>
      </main>
    );
  }
                  ...
                  <div className="space-y-3">
                    {daySessions.map((session) => (
                      <SessionCard
                        ...
                        sessionDetailsLink={`/plan/session/${encodeURIComponent(session.id)}`}
                      />
                    ))}
                  </div>
                ) : hasRest ? (
