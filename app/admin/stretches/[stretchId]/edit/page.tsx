...
export default function EditStretchPage() {
  ...
  useEffect(() => {
    async function loadPageData() {
      if (!stretchId) {
        setErrorMessage("No stretch ID was provided.");
      ...
      const [stretchResult, movementTagsResult, equipmentResult, musclesResult] =
        await Promise.all([
          supabase
            .from("stretches")
            .select(
      ...
      if (stretchResult.error) {
        errors.push(`Could not load stretch: ${stretchResult.error.message}`);
      }
      ...
      if (!stretchResult.error && stretchResult.data) {
        ...
        setSelectedEquipmentOptions(
          equipmentOptions.filter((option) => (stretch.equipment || []).includes(option.slug))
        );

        setSelectedPrimaryMuscles(
          muscleOptions.filter((option) => (stretch.primary_muscles || []).includes(option.slug))
        );

        setSelectedSecondaryMuscles(
          muscleOptions.filter((option) =>
            (stretch.secondary_muscles || []).includes(option.slug)
          )
        );
      }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!trimmedName) {
      setErrorMessage("Please enter a stretch name.");
      setSaving(false);
      return;
    }
    ...
    if (error) {
      setErrorMessage(`Could not update stretch: ${error.message}`);
      setSaving(false);
      return;
    }

    setSuccessMessage("Stretch updated successfully.");
    setSaving(false);
  }
  ...
  if (loadingStretch) {
    return (
      <main style={pageStyle}>
        <div style={cardStyle}>
          <h1 style={titleStyle}>Edit Stretch</h1>
          <p>Loading stretch...</p>
        </div>
      </main>
    );
  ...
  return (
    <main style={pageStyle}>
      <div style={cardStyle}>
        ...
        {stretchId ? (
          <p style={helperStyle}>
            Editing stretch ID: <strong>{stretchId}</strong>
          </p>
        ) : null}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={buttonRowStyle}>
            ...
            <button
              type="button"
              onClick={() => router.push("/admin/stretches")}
