...
export default function CreateStretchPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      if (error.message.toLowerCase().includes("duplicate")) {
        setErrorMessage("A stretch with this name already exists.");
      } else {
        setErrorMessage(`Could not create stretch: ${error.message}`);
      }
      setSaving(false);
      return;
    }
  ...
  return (
    <main style={pageStyle}>
      <div style={cardStyle}>
        <h1 style={titleStyle}>Create Stretch</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={buttonRowStyle}>
            <button type="submit" disabled={saving} style={buttonStyle}>
              {saving ? "Creating..." : "Create Stretch"}
            </button>
...
function makeStretchId(value: string) {
  return value
    .trim()
    .toLowerCase()
    .replace(/['"]/g, "")
