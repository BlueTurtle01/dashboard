...
function pdfValue(value: string | number | null | undefined): string {
  if (value === null || value === undefined || value === "") return "-";
  return String(value);
}

function pdfRaceTime(seconds: number | null | undefined): string {
  if (!seconds || seconds <= 0) return "-";
  return formatSecs(seconds);
}

function pdfKm(value: number | null | undefined, digits = 1): string {
  return value === null || value === undefined ? "-" : `${value.toFixed(digits)} km`;
}

function pdfMeters(value: number | null | undefined): string {
  return value === null || value === undefined ? "-" : `${Math.round(value).toLocaleString()} m`;
}
...
async function exportRaceReadinessPdf({
  result,
  reportAthlete,
  weather,
  results,
  ...
  const athleteName = reportAthlete?.profile.athlete_key ?? "Athlete";
  ...
  const drawHeader = () => {
    ...
    doc.text("Race Readiness Report", marginX, headerY + 5);
  ...
  doc.setFontSize(22);
  doc.setTextColor(30, 58, 30);
  doc.text("Race Readiness Report", marginX, y);
  y += 10;
  doc.setFontSize(15);
  ...
  doc.text(`Athlete: ${athleteName}`, marginX, y);
  y += 12;

  addKeyValues([
    ["Race date", pdfDate(result.race.race_date)],
    ["Distance", pdfKm(result.race.total_distance_km)],
    ["Ascent", pdfMeters(result.race.total_ascent_m)],
    ["Descent", pdfMeters(result.race.total_descent_m)],
    ["Generated", new Date().toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })],
  ]);

  addSection("Executive Summary");
  const scale = expContext?.scale;
  const longestRace = scale?.athlete_max_dist;
  const biggestAscent = scale?.athlete_max_ascent;
  addText(`${athleteName}'s readiness for ${raceName} is assessed from race-history data, course demands, terrain composition, aid logistics, weather history, and physical self-assessment prompts. This generated PDF uses native table pagination so long histories and assessment lists flow across pages cleanly.`);
  addKeyValues([
    ["Distance comparison", scale?.distance_ratio !== undefined ? `${(scale.distance_ratio * 100).toFixed(0)}% of target covered by longest recorded race` : "No comparable athlete distance data"],
    ["Ascent comparison", scale?.ascent_ratio !== undefined ? `${(scale.ascent_ratio * 100).toFixed(0)}% of target ascent covered by biggest recorded race` : "No comparable athlete ascent data"],
    ["Longest race on record", longestRace ? `${longestRace.race_name} (${longestRace.year}) - ${pdfKm(longestRace.km)}` : "-"],
    ["Largest ascent on record", biggestAscent ? `${biggestAscent.race_name} (${biggestAscent.year}) - ${pdfMeters(biggestAscent.m)}` : "-"],
    ["Elevation profile match", elevProfileMatch?.best_match ? `${elevProfileMatch.best_match.race_name} (${elevProfileMatch.best_match.year}) - ${elevProfileMatch.best_match.similarity_pct.toFixed(0)}% similar` : "-"],
  ]);

  addSection("Course Profile");
  ...
  addKeyValues([
    ["Distance", pdfKm(result.race.total_distance_km)],
    ["Ascent / descent", `${pdfMeters(result.race.total_ascent_m)} / ${pdfMeters(result.race.total_descent_m)}`],
    ["Terrain sections", String(result.terrain_sections.length)],
    ["Aid stations", String(result.aid_stations?.length ?? 0)],
    ["Typical weather", avgMaxTemp !== null ? `${avgMinTemp?.toFixed(1)}-${avgMaxTemp.toFixed(1)} C, ${avgPrecip?.toFixed(1)} mm precip, ${wetPct}% wet days` : "No weather history available"],
    ["Latest results data", results?.has_data ? `${results.total_finishers ?? 0} finishers in ${results.latest_year ?? "-"}` : "No results data available"],
    ["Split analysis", splitAnalysis?.halfway_analysis ? `${splitAnalysis.halfway_analysis.label}, typical halfway ${splitAnalysis.halfway_analysis.typical_ratio.toFixed(2)} of total time` : "-"],
  ]);

  addTable(
    "Terrain Sections",
    ["Start", "End", "Km", "Type", "Terrain", "Gradient", "Ascent", "Descent"],
    result.terrain_sections.map(s => [
      s.start_km.toFixed(1),
      s.end_km.toFixed(1),
      s.distance_km.toFixed(1),
      s.section_type.replaceAll("_", " "),
      s.terrain.replaceAll("_", " "),
      `${s.avg_gradient_percent.toFixed(1)}%`,
      Math.round(s.ascent_m),
      Math.round(s.descent_m),
    ]),
    { 3: { cellWidth: 25 }, 4: { cellWidth: 28 } },
  );

  if (reportAthlete) {
    addSection("Athlete Profile");
    addKeyValues([
      ["Athlete", reportAthlete.profile.athlete_key],
      ["Club", pdfValue(reportAthlete.profile.club)],
      ["Age group", pdfValue(reportAthlete.profile.age_group)],
      ["Race count", String(reportAthlete.profile.race_count)],
      ["Finishes", String(reportAthlete.profile.finish_count)],
      ["DNF count / rate", `${reportAthlete.profile.dnf_count} / ${reportAthlete.profile.dnf_rate !== null ? `${(reportAthlete.profile.dnf_rate * 100).toFixed(1)}%` : "-"}`],
      ["Career span", `${pdfValue(reportAthlete.profile.first_result_year)}-${pdfValue(reportAthlete.profile.last_result_year)}`],
      ["Max flat equivalent", pdfKm(reportAthlete.profile.max_flat_equiv_km)],
    ]);

    const finished = reportAthlete.races
      .filter(r => r.result_status === "FINISHED" || r.result_status === "UNKNOWN")
      .sort((a, b) => b.result_year - a.result_year);
    addTable(
      "Finished Race History - Demands vs Target",
      ["Race", "Year", "Time", "Distance", "Vs target", "Ascent", "Vs target", "Position", "Category"],
      finished.map(r => {
        const distRatio = r.total_distance_km && result.race.total_distance_km ? r.total_distance_km / result.race.total_distance_km : null;
        const ascentRatio = r.total_ascent_m && result.race.total_ascent_m ? r.total_ascent_m / result.race.total_ascent_m : null;
        return [
          r.race_name,
          r.result_year,
          pdfRaceTime(r.finish_seconds),
          pdfKm(r.total_distance_km, 0),
          distRatio !== null ? `${(distRatio * 100).toFixed(0)}%` : "-",
          pdfMeters(r.total_ascent_m),
          ascentRatio !== null ? `${(ascentRatio * 100).toFixed(0)}%` : "-",
          r.position ? `${r.position}${r.total_finishers ? `/${r.total_finishers}` : ""}` : "-",
          r.cat_position ? `${r.cat_position}${r.cat_finishers ? `/${r.cat_finishers}` : ""}` : "-",
        ];
      }),
      { 0: { cellWidth: 42 }, 2: { cellWidth: 17 }, 3: { cellWidth: 18 }, 5: { cellWidth: 20 } },
    );

    addTable(
      "Athlete Terrain Experience",
      ["Gradient type", "Surface", "Km completed", "Races", "Average gradient"],
      (reportAthlete.terrain_pairings ?? []).map(p => [
        p.section_type.replaceAll("_", " "),
        p.terrain.replaceAll("_", " "),
        p.total_km.toFixed(1),
        p.race_count,
        `${p.avg_gradient > 0 ? "+" : ""}${p.avg_gradient.toFixed(1)}%`,
      ]),
    );
  }
  ...
  addTable(
    ...
    ["Race", "Date", "Distance", "Ascent", "Distance from base", "Gap coverage", "Main gaps filled"],
    (prepRaces?.suggestions ?? []).map(s => [
      s.race_name,
      pdfDate(s.next_date),
      pdfKm(s.total_distance_km),
      pdfMeters(s.total_ascent_m),
      `${s.distance_miles.toFixed(1)} mi`,
      `${s.gap_fill_score.toFixed(0)}%`,
      s.gaps_filled.map(g => `${g.section_type.replaceAll("_", " ")} ${g.terrain.replaceAll("_", " ")} (${g.race_km.toFixed(1)}/${g.needed_km.toFixed(1)} km)`).join("; "),
    ]),
    { 0: { cellWidth: 38 }, 6: { cellWidth: 50 } },
  );

  addTable(
    "Physical Self-Assessments",
    ["Category", "Assessment", "Identifies", "Instructions", "Record"],
    assessmentTests.map(t => [
      t.category ?? "general",
      t.name,
      t.aim ?? t.description ?? "-",
      t.instructions.length > 0 ? t.instructions.map((step, i) => `${i + 1}. ${step}`).join("\n") : "-",
      t.what_to_record ?? t.notes ?? "-",
    ]),
    { 0: { cellWidth: 21 }, 1: { cellWidth: 33 }, 2: { cellWidth: 40 }, 3: { cellWidth: 58 }, 4: { cellWidth: 30 } },
  );

  if (reportAthlete) {
    addTable(
      "Full Race History",
      ["Year", "Race", "Status", "Time", "Distance", "Ascent", "Overall", "Category"],
      [...reportAthlete.races].sort((a, b) => b.result_year - a.result_year).map(r => [
        r.result_year,
        r.race_name,
        r.result_status,
        pdfRaceTime(r.finish_seconds),
      ...
      { 1: { cellWidth: 48 }, 3: { cellWidth: 18 }, 4: { cellWidth: 20 }, 5: { cellWidth: 22 } },
    );
  }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text("Generated from structured report data - tables paginate natively", marginX, pageH - 8);
  }

  const safeRaceName = raceName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "race";
  const safeAthleteName = athleteName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "athlete";
  doc.save(`${safeAthleteName}-${safeRaceName}-readiness.pdf`);
}
...
export default function RaceReadinessPage() {
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      console.error("[race-readiness/pdf-export]", err);
      setGenError("Failed to generate PDF export.");
    } finally {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f9f9f9" }}>
      ...
      <div className="no-print" style={{ background: "#fff", borderBottom: "1px solid #e5e5e5", padding: "24px 32px", display: "flex", flexWrap: "wrap", gap: "20px", alignItems: "flex-end" }}>
        ...
        {result && (
          <button type="button" onClick={handleExportPdf} disabled={pdfExporting} style={{ ...printBtn, opacity: pdfExporting ? 0.65 : 1, cursor: pdfExporting ? "default" : "pointer" }}>
            {pdfExporting ? "Generating PDF..." : "Export to PDF"}
          </button>
        )}
